...
namespace Work_Bank_Api.Dtos
{
    public class CreateTtansactionDto
    {
        ...
        [RegularExpression("^[\\u05D0-\\u05EA\\s'-]{1,20}$",ErrorMessage = "HebrewName should contain only hebrew laters")]
        public string HebrewName { get; set; } = string.Empty;
       
        [Required]
        [MaxLength(15, ErrorMessage = " Name Should Contanin no more than 20 Character.")]
        [RegularExpression("^[A-Za-z\\s'-]{1,15}$",ErrorMessage = "EnglishName should contain only english latters")]
